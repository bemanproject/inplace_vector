...
/** The return type of `operator->()` in a proxy iterator.
    ...
    `T`. */
template <typename T>
#if defined(BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_DOXYGEN) ||                   \
...
auto make_pointer(T &&value,
                  std::enable_if_t<std::is_pointer<Pointer>::value &&
...
template <typename Pointer, typename IteratorConcept> struct pointer {
  using type = Pointer;
};
template <typename Pointer> struct pointer<Pointer, std::output_iterator_tag> {
  using type = void;
};
...
using interoperable =
    std::integral_constant<bool, (std::is_convertible<T, U>::value ||
...
template <typename AlwaysVoid, template <class...> class Template,
          typename... Args>
...
template <typename T, typename U> struct common_eq<T, U, true> {
  static constexpr auto call(T lhs, U rhs) {
...
template <typename T, typename U>
constexpr auto
common_diff(T lhs, U rhs) noexcept(noexcept(static_cast<common_t<T, U>>(lhs) -
                                            static_cast<common_t<T, U>>(rhs)))
    -> decltype(static_cast<common_t<T, U>>(lhs) -
                static_cast<common_t<T, U>>(rhs)) {
...
BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_NAMESPACE_V1 {
  ...
  template <typename Derived, typename IteratorConcept, typename ValueType,
            typename Reference = ValueType &, typename Pointer = ValueType *,
  ...
  struct plus_eq<Iterator, DifferenceType,
                 void_t<decltype(std::declval<Iterator &>() +=
                                 std::declval<DifferenceType>())>>
      : std::true_type {};
  ...
  void derived_iterator(
      const iterator_interface<D, IteratorConcept, ValueType, Reference,
  ...
  public:
    ...
    constexpr auto operator*() const
        noexcept(noexcept(*access::base(std::declval<const D &>())))
    ...
    constexpr auto operator->() const noexcept(noexcept(
        detail::make_pointer<pointer, reference>(*std::declval<const D &>())))
        -> decltype(detail::make_pointer<pointer, reference>(
            *std::declval<const D &>())) {
      return detail::make_pointer<pointer, reference>(*derived());
    }
    ...
    constexpr auto operator[](difference_type i) const
        noexcept(noexcept(D(std::declval<const D &>()),
                          std::declval<D &>() += i, *std::declval<D &>()))
    ...
    template <typename D = Derived>
    constexpr auto operator++() noexcept(noexcept(std::declval<D &>() +=
                                                  difference_type(1)))
        -> decltype(std::declval<D &>() += difference_type(1),
                    std::declval<D &>()) {
      derived() += difference_type(1);
      return derived();
    }
    template <typename D = Derived>
    constexpr auto operator++(int) noexcept(noexcept(D(std::declval<D &>()),
                                                     ++std::declval<D &>()))
        -> std::remove_reference_t<decltype(D(std::declval<D &>()),
                                            ++std::declval<D &>(),
                                            std::declval<D &>())> {
      D retval = derived();
      ++derived();
  ...
  constexpr auto operator==(IteratorInterface1 lhs,
                            IteratorInterface2 rhs) noexcept
  ...
  constexpr auto operator==(
      ...
      ->decltype(v1_dtl::derived_iterator(lhs),
                 detail::common_diff(lhs, rhs) == 0) {
...
BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_NAMESPACE_V2 {
  ...
  namespace v2_dtl {
  template <typename Iterator> struct iter_concept;
  ...
  template <
      typename IteratorConcept, typename ReferenceType,
      typename IteratorCategory =
          decltype(v2_dtl::category_tag<IteratorConcept, ReferenceType>())>
...
/** `static_asserts` that type `type` models concept `concept_name`.  This is
    ...
    For example:
   `BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_CONCEPT(my_iter,
    std::input_iterator)`.

    \note This macro expands to nothing when `__cpp_lib_concepts` is not
    defined. */
#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_CONCEPT(            \
    type, concept_name)
...
/** `static_asserts` that the types of all typedefs in
    ...
    For example:
   `BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_TRAITS(my_iter,
    std::input_iterator_tag, std::input_iterator, int, int &, int *,
   std::ptrdiff_t)`.
...
#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_CONCEPT_IMPL( \
    type, concept_name)                                                           \
...
#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_CONCEPT(            \
    iter, concept_name)                                                        \
  BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_CONCEPT_IMPL(    \
      iter, concept_name)
#else
#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_CONCEPT(            \
    iter, concept_name)
...
#define BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_STATIC_ASSERT_ITERATOR_TRAITS_IMPL( \
  ...
  static_assert(std::is_same<typename std::iterator_traits<iter>::reference,     \
                ...
                "");                                                             \
  static_assert(                                                                 \
      std::is_same<typename std::iterator_traits<iter>::pointer, ptr>::value,    \
      "");                                                                       \
  static_assert(                                                                 \
