...
BEMAN_OPTIONAL_DETAIL_STL_INTERFACES_NAMESPACE_V1 {
  ...
  template <typename Range> using iterator_t = typename iterator<Range>::type;
  ...
  template <typename Range> using sentinel_t = typename sentinel<Range>::type;
  ...
  struct decrementable_sentinel<
      Range, void_t<decltype(--std::declval<sentinel_t<Range> &>())>>
      : std::true_type {};
  } // namespace v1_dtl
}
} // namespace stl_interfaces
