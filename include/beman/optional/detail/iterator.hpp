...
// Forward declaration.
template <class T, class Container> struct contiguous_iterator;
...
using base_contiguous_iterator = stl_interfaces::iterator_interface<
    ...
    contiguous_iterator<T, Container>, // Required for P2727R4 to work with
                                       // C++20/C++23. TODO: Do more
                                       // experiments.
#endif
    std::contiguous_iterator_tag, T>;

// This is a minimal contiguous iterator.  It uses stl_interfaces library from
// Boost (current implementation based on https://wg21.link/P2727R4).
//
// TODO: Change this to use the stl_interfaces library from Beman if/when
// available.
//
// @tparam T - The type of the elements the iterator points to.
// @tparam Container - The type of the container the iterator points to. This
// parameter exists solely so that different containers using this template can
// instantiate different types, even if the T parameter is the same.
template <class T, class Container>
struct contiguous_iterator : public base_contiguous_iterator<T, Container> {
  ...
  // As per P2727R4, for contiguous iterator we only need to provide operator*,
  ...
  operator-(contiguous_iterator other) const noexcept {
