...
template <class T>
inline constexpr bool std::ranges::enable_view<beman::optional::optional<T>> =
    true;

// Iterators for optional<T&> have lifetimes that are not tied to the optional.
template <class T>
inline constexpr bool
    std::ranges::enable_borrowed_range<beman::optional::optional<T &>> = true;

// Since P3168R2: Give std::optional Range Support.
#if defined(__cpp_lib_format_ranges)
template <class T>
inline constexpr auto std::format_kind<beman::optional::optional<T>> =
...
template <typename T> inline constexpr bool is_optional<optional<T>> = true;
} // namespace detail
/**
...
private:
  friend constexpr bool operator==(nullopt_t, nullopt_t) noexcept {
    return true;
  }

  friend constexpr std::strong_ordering operator<=>(nullopt_t,
                                                    nullopt_t) noexcept {
...
template <class T>
constexpr std::strong_ordering operator<=>(const optional<T> &,
                                           nullopt_t) noexcept;
...
constexpr optional<std::decay_t<T>> make_optional(T &&) noexcept(
...
public:
  ...
  /**
   * @brief  Assigns the contained value from \p u, destroying the old value if
   * there
   *
  ...
  /**
   * @brief   Assigns the contained value from \p rhs if it has one, destroying
   * the old value if there
   *
   */
  template <class U>
  ...
  /**
   * @brief Constructs the value in-place using the given arguments, destroying
   * the current one if there
   *
   * @tparam U
   * @tparam Args
  ...
  /**
   * @brief Converts the optional to a boolean indicating whether it has a
   * value.
   *
   * @return bool
   */
   ...
   * the result of applying \p f to the optional if it does not.
   *
  ...
  /**
   * @brief Resets the optional to an empty state, destroying the contained
...
template <class T> inline constexpr optional<T>::optional(nullopt_t) noexcept {}
...
template <class T>
template <class U, class... Args>
inline constexpr optional<T>::optional(in_place_t, std::initializer_list<U> il,
                                       Args &&...args)
  requires std::is_constructible_v<T, std::initializer_list<U> &, Args &&...>
...
inline constexpr optional<T> &optional<T>::operator=(
    optional<T> &&rhs) noexcept(std::is_nothrow_move_constructible_v<T>)
  requires std::is_move_constructible_v<T> && std::is_move_assignable_v<T> &&
           (!std::is_trivially_move_assignable_v<T>)
{
  if (!rhs.has_value())
...
template <class T> inline constexpr const T *optional<T>::operator->() const {
  return std::addressof(value_);
}

template <class T> inline constexpr T *optional<T>::operator->() {
  return std::addressof(value_);
}
...
/// bad_optional_access
template <class T> inline constexpr T &optional<T>::value() & {
  return has_value() ? value_ : throw bad_optional_access();
}
template <class T> inline constexpr const T &optional<T>::value() const & {
  return has_value() ? value_ : throw bad_optional_access();
}
template <class T> inline constexpr T &&optional<T>::value() && {
...
inline constexpr std::remove_cv_t<T> optional<T>::value_or(U &&u) const & {
  ...
  static_assert(std::is_convertible_v<U, X>,
                "Must be able to convert u to remove_cv_t<T>");
  if (has_value())
    return value_;
...
constexpr auto optional<T>::transform(F &&f) & {
  ...
  static_assert(std::is_object_v<U> ||
                std::is_reference_v<U>); /// References now allowed
  return (has_value())
             ? optional<U>{detail::from_function, std::forward<F>(f), value_}
             : optional<U>{};
}
...
constexpr auto optional<T>::transform(F &&f) && {
  ...
  static_assert(std::is_object_v<U> ||
                std::is_reference_v<U>); /// References now allowed
  return (has_value()) ? optional<U>{detail::from_function, std::forward<F>(f),
                                     std::move(value_)}
                       : optional<U>{};
}
...
{
  static_assert(
      std::is_same_v<std::remove_cvref_t<std::invoke_result_t<F>>, optional>);
  if (has_value())
    return value_;
...
template <class T> constexpr void optional<T>::reset() noexcept {
...
constexpr optional<std::decay_t<T>> make_optional(T &&t) noexcept(
    std::is_nothrow_constructible_v<optional<std::decay_t<T>>, T>)
  requires std::is_constructible_v<std::decay_t<T>, T>
{
  ...
  requires std::is_constructible_v<T, Args...>
{
  static_assert(!std::is_reference_v<T>,
                "May not make an optional containing a reference");
  return optional<T>{in_place, std::forward<Args>(args)...};
}

template <typename T, typename U, typename... Args>
constexpr optional<T>
make_optional(std::initializer_list<U> init_list, Args &&...args) noexcept(
    std::is_nothrow_constructible_v<T, std::initializer_list<U> &, Args...>)
  requires std::is_constructible_v<T, std::initializer_list<U> &, Args...>
{
  static_assert(!std::is_reference_v<U>,
                "Can not make optional from initializer_list of references.");
...
private:
  ...
  template <class U> constexpr void convert_ref_init_val(U &&u) {
...
// template <class T>
// template <class U>
//     requires(std::is_constructible_v<T&, U> && !(is_same_v<remove_cvref_t<U>,
//     in_place_t>) &&
...
template <class T> constexpr T *optional<T &>::operator->() const noexcept {
  return value_;
}
...
template <class T> constexpr optional<T &>::operator bool() const noexcept {
  return value_ != nullptr;
}
template <class T> constexpr bool optional<T &>::has_value() const noexcept {
  return value_ != nullptr;
}

template <class T> constexpr T &optional<T &>::value() const {
  if (has_value()) {
    return *value_;
  }
  throw bad_optional_access();
...
constexpr std::decay_t<T> optional<T &>::value_or(U &&u) const {
  using X = std::remove_cv_t<T>;
  static_assert(std::is_convertible_v<T &, X>,
                "remove_cv_t<T> must be constructible from a T&");
  static_assert(std::is_convertible_v<U, X>,
                "Must be able to convert u to remove_cv_t<T>");
  if (has_value()) {
    return *value_;
