...
configure beman.inplace_vector to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_INPLACE_VECTOR`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_INPLACE_VECTOR" -C 2
```
...
### `BEMAN_INPLACE_VECTOR_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.inplace_vector` can use
`find_package(beman.inplace_vector)` to locate the library.
